...
def dict_from_matrix(matrix, atoms=10, alpha=1, n_iter=50):
    """
    :param matrix:  (tot_descriptors X tot features per descriptors)
...
def get_y_tag_from_descriptors_matrix(anchors_descriptors, dict:skl.decomposition.MiniBatchDictionaryLearning, V, transform_algorithm, **kwargs):
    """
     this fucntion is used for the Feature Coding Layers
    ...
    anchor_rep_matrix = np.dot(anchor_coeffs, V)
    # now anchor_rep_matrix is of shape (num_of_anchors X 128)
    y_tag = anchor_rep_matrix.sum(axis=0)
    return y_tag
