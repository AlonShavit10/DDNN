...
config.gpu_options.per_process_gpu_memory_fraction = 0.1  # 0.38
session = tf.compat.v1.Session(config=config)
def train_model(base_folder,run_name):


    tf.compat.v1.keras.backend.set_session(session)
    class_train_images_array, class_test_images_array = get_train_test_images(params.IMAGES_TRAIN_PER_CLASS, params.IMAGES_TEST_PER_CLASS)
    ...
    history = model.fit_generator(train_generator,
                                  ...
                                  validation_steps=max(params.IMAGES_TEST_PER_CLASS * 10 // params.batch, 1))
